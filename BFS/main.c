...
int numOfVertex;
int adj[MAX][MAX];          //adjacency matrix, in which adj[i][j] = 1, denotes there is an  edge from i to j
...
void create_graph() {
    ...
    max_edge = numOfVertex * (numOfVertex - 1);         //assuming each vertex has an edge with remaining (n-1) vertices
    ...
    for (count = 1; count <= max_edge; count++)
    {
        ...
        if (origin >= numOfVertex || destin >= numOfVertex || origin < 0 || destin < 0) {
            printf("Invalid edge!\n");
            count--;
        } else {
            adj[origin][destin] = 1;
